...
QueueHandle_t xQueuehandle;//gera ponteiro da fila
const uint8_t queueSize = 10;// vai ter 10 intens na fila
typedef struct {
...
int main( void ){
    ...
    //==============================================
		...
		xQueuehandle = xQueueCreate(queueSize, sizeof(Message));// tamanho da cada item da fila
...
static void vTaskMandaUART(void * pvParameters)//vai receber da fila
		{
			while(1)
			{
				...
				UART_Send(LPC_UART0, (uint8_t *)receivedData.message, strlen(receivedData.message), BLOCKING);
			}
		}
					...
					while(1)
					{
						if(!(GPIO_ReadValue(botao_PORT) & (1 << botao_BIT)) )
						{
							if(filtroBotao < 10)
							{
								...
								if(filtroBotao == 10)
								{
									//add na fila a mensagem, antes aqui tinha o uart send
									strcpy(msgToSend.message, "Hello, world!"); ///////////// parei aqui depois de colocar a declaração q é do tipo struct mensagem ali em cima
									xQueueSend(xQueuehandle, &msgToSend, portMAX_DELAY);
								}
